...
from bwperim_2 import bwperim
from ellipses import LSqEllipse #The code is pulled frm https://github.com/bdhammel/least-squares-ellipse-fitting
# from skimage.draw import ellipse_perimeter


def isolate_islands(prediction, threshold=0.5):
    
    # bw = closing(prediction > threshold , square(3))
    bw = np.zeros_like(prediction,dtype=np.uint8)
    bw[prediction > threshold] = 255

    bw = cv2.morphologyEx(bw, cv2.MORPH_CLOSE,(3,3))
    # labelled = label(bw)  
    # retval, labelled, stats, centroids = cv2.connectedComponentsWithStats(bw,connectivity=4)
    retval, labelled, stats, centroids = cv2.connectedComponentsWithStats(bw)
    ...
    # regionprops's property ==> area, bbox_area, image, bbox, etc. 
    ...
    # output = np.zeros(labelled.shape, dtype=np.float32)
    output = np.zeros(labelled.shape, dtype=np.uint8)
    # output = np.zeros(labelled.shape)
    if max_label == 0:
        return output
    else:
        # output[labelled == max_label] = 1
        output[labelled == max_label] = 255
        return output
...
def gen_ellipse_contour_perim_compact(perim): 
    ...
    if max_value == 0:
        return None
    input_points = np.where(perim == max_value)
    # input_points = np.where(perim == 255)
    # input_points = np.where(perim == 1)
